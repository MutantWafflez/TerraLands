...
namespace TerraLands
{
    public class TerraLands : Mod
    {
        ...
        private double NPC_StrikeNPC(On.Terraria.NPC.orig_StrikeNPC orig, NPC self, int Damage, float knockBack, int hitDirection, bool crit, bool noEffect, bool fromNet)
        {
			...
			double damagePlaceholder = (double)Damage;
			...
			if (NPCLoader.StrikeNPC(self, ref damagePlaceholder, defensePlaceholder, ref knockBack, hitDirection, ref crit))
			{
				...
				{
					damagePlaceholder *= (double)self.takenDamageMultiplier;
				}
			}
			...
			if (damagePlaceholder >= 1.0)
			{
				...
				if (self.townNPC)
				{
					if (self.aiStyle == 7 && (self.ai[0] == 3f || self.ai[0] == 4f || self.ai[0] == 16f || self.ai[0] == 17f))
					{
						...
						{
							chosenNPC.ai[0] = 1f;
							chosenNPC.ai[1] = (float)(300 + Main.rand.Next(300));
							chosenNPC.ai[2] = 0f;
					...
					self.ai[0] = 1f;
					self.ai[1] = (float)(300 + Main.rand.Next(300));
					self.ai[2] = 0f;
					self.localAI[3] = 0f;
				...
				if (self.type == NPCID.SantaNK1 && (double)self.life >= (double)self.lifeMax * 0.5 && (double)self.life - damagePlaceholder < (double)self.lifeMax * 0.5)
				{
				...
				if (knockBack > 0f && self.knockBackResist > 0f)
				{
					...
					else
					{
						...
						self.velocity.X = num3 * (float)hitDirection * self.knockBackResist;
					}
				}
